// Copyright 2026 Knodex Authors
// SPDX-License-Identifier: AGPL-3.0-only

package cel

import (
	"testing"

	"github.com/knodex/knodex/server/internal/models"
)

func TestAnalyzeCondition(t *testing.T) {
	tests := []struct {
		name          string
		expr          string
		wantEvaluable bool
		wantRuleCount int
		wantRules     []models.ConditionRule // nil means don't check individual rules
	}{
		{
			name:          "simple boolean true",
			expr:          "schema.spec.enabled == true",
			wantEvaluable: true,
			wantRuleCount: 1,
			wantRules: []models.ConditionRule{
				{Field: "spec.enabled", Op: "==", Value: true},
			},
		},
		{
			name:          "simple boolean false",
			expr:          "schema.spec.enabled == false",
			wantEvaluable: true,
			wantRuleCount: 1,
			wantRules: []models.ConditionRule{
				{Field: "spec.enabled", Op: "==", Value: false},
			},
		},
		{
			name:          "numeric greater than",
			expr:          "schema.spec.replicas > 0",
			wantEvaluable: true,
			wantRuleCount: 1,
			wantRules: []models.ConditionRule{
				{Field: "spec.replicas", Op: ">", Value: int64(0)},
			},
		},
		{
			name:          "string comparison",
			expr:          `schema.spec.mode == "advanced"`,
			wantEvaluable: true,
			wantRuleCount: 1,
			wantRules: []models.ConditionRule{
				{Field: "spec.mode", Op: "==", Value: "advanced"},
			},
		},
		{
			name:          "AND chain with two comparisons",
			expr:          "schema.spec.a == true && schema.spec.b == true",
			wantEvaluable: true,
			wantRuleCount: 2,
		},
		{
			name:          "bare field reference (truthy check)",
			expr:          "schema.spec.enabled",
			wantEvaluable: true,
			wantRuleCount: 1,
			wantRules: []models.ConditionRule{
				{Field: "spec.enabled", Op: "==", Value: true},
			},
		},
		{
			name:          "OR expression - not evaluable",
			expr:          "schema.spec.a == true || schema.spec.b == true",
			wantEvaluable: false,
			wantRuleCount: 0,
		},
		{
			name:          "NOT expression - not evaluable",
			expr:          "!schema.spec.enabled",
			wantEvaluable: false,
			wantRuleCount: 0,
		},
		{
			name:          "with delimiters",
			expr:          "${schema.spec.enabled == true}",
			wantEvaluable: true,
			wantRuleCount: 1,
			wantRules: []models.ConditionRule{
				{Field: "spec.enabled", Op: "==", Value: true},
			},
		},
		{
			name:          "resource reference - not evaluable",
			expr:          "database.status.ready == true",
			wantEvaluable: false,
			wantRuleCount: 0,
		},
		{
			name:          "comprehension - not evaluable",
			expr:          `schema.spec.list.exists(x, x == "foo")`,
			wantEvaluable: false,
			wantRuleCount: 0,
		},
		{
			name:          "empty string",
			expr:          "",
			wantEvaluable: false,
			wantRuleCount: 0,
		},
		{
			name:          "AND with mixed operators",
			expr:          `schema.spec.a == true && schema.spec.b != "x"`,
			wantEvaluable: true,
			wantRuleCount: 2,
		},
		{
			name:          "reversed operand - literal first",
			expr:          "true == schema.spec.enabled",
			wantEvaluable: true,
			wantRuleCount: 1,
			wantRules: []models.ConditionRule{
				{Field: "spec.enabled", Op: "==", Value: true},
			},
		},
		{
			name:          "reversed inequality - literal first",
			expr:          `3 > schema.spec.replicas`,
			wantEvaluable: true,
			wantRuleCount: 1,
			wantRules: []models.ConditionRule{
				{Field: "spec.replicas", Op: "<", Value: int64(3)},
			},
		},
		{
			name:          "invalid CEL - no panic",
			expr:          "invalid CEL ==}",
			wantEvaluable: false,
			wantRuleCount: 0,
		},
		{
			name:          "greater than or equal",
			expr:          "schema.spec.replicas >= 3",
			wantEvaluable: true,
			wantRuleCount: 1,
			wantRules: []models.ConditionRule{
				{Field: "spec.replicas", Op: ">=", Value: int64(3)},
			},
		},
		{
			name:          "less than or equal",
			expr:          "schema.spec.replicas <= 10",
			wantEvaluable: true,
			wantRuleCount: 1,
			wantRules: []models.ConditionRule{
				{Field: "spec.replicas", Op: "<=", Value: int64(10)},
			},
		},
		{
			name:          "reversed >= becomes <=",
			expr:          "5 >= schema.spec.replicas",
			wantEvaluable: true,
			wantRuleCount: 1,
			wantRules: []models.ConditionRule{
				{Field: "spec.replicas", Op: "<=", Value: int64(5)},
			},
		},
		{
			name:          "reversed <= becomes >=",
			expr:          "1 <= schema.spec.replicas",
			wantEvaluable: true,
			wantRuleCount: 1,
			wantRules: []models.ConditionRule{
				{Field: "spec.replicas", Op: ">=", Value: int64(1)},
			},
		},
		{
			name:          "float literal",
			expr:          "schema.spec.threshold > 3.14",
			wantEvaluable: true,
			wantRuleCount: 1,
			wantRules: []models.ConditionRule{
				{Field: "spec.threshold", Op: ">", Value: float64(3.14)},
			},
		},
		{
			name:          "bare identifier - not evaluable",
			expr:          "enabled",
			wantEvaluable: false,
			wantRuleCount: 0,
		},
		{
			name:          "reversed < becomes >",
			expr:          "3 < schema.spec.replicas",
			wantEvaluable: true,
			wantRuleCount: 1,
			wantRules: []models.ConditionRule{
				{Field: "spec.replicas", Op: ">", Value: int64(3)},
			},
		},
		{
			name:          "function call size - not evaluable",
			expr:          "size(schema.spec.list)",
			wantEvaluable: false,
			wantRuleCount: 0,
		},
		{
			name:          "uint literal",
			expr:          "schema.spec.count == 3u",
			wantEvaluable: true,
			wantRuleCount: 1,
			wantRules: []models.ConditionRule{
				{Field: "spec.count", Op: "==", Value: uint64(3)},
			},
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			evaluable, rules := AnalyzeCondition(tt.expr)

			if evaluable != tt.wantEvaluable {
				t.Errorf("AnalyzeCondition(%q) evaluable = %v, want %v", tt.expr, evaluable, tt.wantEvaluable)
			}

			if len(rules) != tt.wantRuleCount {
				t.Errorf("AnalyzeCondition(%q) rule count = %d, want %d, rules: %+v",
					tt.expr, len(rules), tt.wantRuleCount, rules)
			}

			// Check individual rules if specified
			if tt.wantRules != nil && len(rules) == len(tt.wantRules) {
				for i, want := range tt.wantRules {
					got := rules[i]
					if got.Field != want.Field {
						t.Errorf("rule[%d].Field = %q, want %q", i, got.Field, want.Field)
					}
					if got.Op != want.Op {
						t.Errorf("rule[%d].Op = %q, want %q", i, got.Op, want.Op)
					}
					if got.Value != want.Value {
						t.Errorf("rule[%d].Value = %v (%T), want %v (%T)",
							i, got.Value, got.Value, want.Value, want.Value)
					}
				}
			}
		})
	}
}

func TestStripDelimiters(t *testing.T) {
	tests := []struct {
		input string
		want  string
	}{
		{"${schema.spec.enabled == true}", "schema.spec.enabled == true"},
		{"schema.spec.enabled == true", "schema.spec.enabled == true"},
		{"  ${  inner  }  ", "inner"},
		{"${ }", ""},
		{"", ""},
		{"${hello", "${hello"}, // open delimiter without close
		{"hello}", "hello}"},   // close delimiter without open
	}

	for _, tt := range tests {
		t.Run(tt.input, func(t *testing.T) {
			got := stripDelimiters(tt.input)
			if got != tt.want {
				t.Errorf("stripDelimiters(%q) = %q, want %q", tt.input, got, tt.want)
			}
		})
	}
}

func TestAnalyzeCondition_NestedFieldPath(t *testing.T) {
	evaluable, rules := AnalyzeCondition("schema.spec.ingress.tls.enabled == true")

	if !evaluable {
		t.Fatal("expected client-evaluable")
	}

	if len(rules) != 1 {
		t.Fatalf("expected 1 rule, got %d", len(rules))
	}

	if rules[0].Field != "spec.ingress.tls.enabled" {
		t.Errorf("field = %q, want %q", rules[0].Field, "spec.ingress.tls.enabled")
	}
}

func TestAnalyzeCondition_NotEqualOperator(t *testing.T) {
	evaluable, rules := AnalyzeCondition(`schema.spec.tier != "free"`)

	if !evaluable {
		t.Fatal("expected client-evaluable")
	}

	if len(rules) != 1 {
		t.Fatalf("expected 1 rule, got %d", len(rules))
	}

	if rules[0].Op != "!=" {
		t.Errorf("op = %q, want %q", rules[0].Op, "!=")
	}

	if rules[0].Value != "free" {
		t.Errorf("value = %v, want %q", rules[0].Value, "free")
	}
}

func TestAnalyzeCondition_MultipleDelimitedExpressions(t *testing.T) {
	expr := "${schema.spec.ingress.enabled == true}"
	evaluable, rules := AnalyzeCondition(expr)

	if !evaluable {
		t.Fatalf("expected client-evaluable for %q", expr)
	}

	if len(rules) != 1 {
		t.Fatalf("expected 1 rule, got %d", len(rules))
	}
}

// Tests for Knodex wrapper functions around KRO CEL types.

func TestNewExpression_Wrapper(t *testing.T) {
	expr := NewExpression("schema.spec.enabled == true")

	if expr.Original != "schema.spec.enabled == true" {
		t.Errorf("Original = %q", expr.Original)
	}
	if expr.References != nil {
		t.Error("References should be nil for uncompiled expression")
	}
	if expr.Program != nil {
		t.Error("Program should be nil for uncompiled expression")
	}
}

func TestNewExpressionSlice_Wrapper(t *testing.T) {
	exprs := NewExpressionSlice("a", "b", "c")

	if len(exprs) != 3 {
		t.Fatalf("expected 3 expressions, got %d", len(exprs))
	}
	for i, want := range []string{"a", "b", "c"} {
		if exprs[i].Original != want {
			t.Errorf("exprs[%d].Original = %q, want %q", i, exprs[i].Original, want)
		}
	}
}

func TestNewUntypedEnvironment_Wrapper(t *testing.T) {
	env, err := NewUntypedEnvironment([]string{"schema", "deployment"})
	if err != nil {
		t.Fatalf("NewUntypedEnvironment() error = %v", err)
	}

	astResult, issues := env.Parse("schema.spec.name")
	if issues != nil && issues.Err() != nil {
		t.Fatalf("Parse() error = %v", issues.Err())
	}
	if astResult == nil {
		t.Fatal("Parse() returned nil AST")
	}
}

func TestNewTypedEnvironment_Wrapper(t *testing.T) {
	env, err := NewTypedEnvironment(nil)
	if err != nil {
		t.Fatalf("NewTypedEnvironment(nil) error = %v", err)
	}

	astResult, issues := env.Parse("1 + 1")
	if issues != nil && issues.Err() != nil {
		t.Fatalf("Parse() error = %v", issues.Err())
	}
	if astResult == nil {
		t.Fatal("Parse() returned nil AST")
	}
}
