...
func (h *SecretsHandler) SetLicenseService(ls services.LicenseService) {
	h.licenseService = ls
}

// checkSecretsLicense validates the license for the secrets feature.
// Returns false and writes an error if the license check fails.
func (h *SecretsHandler) checkSecretsLicense(w http.ResponseWriter, isWrite bool) bool {
	featureDetail := map[string]string{"feature": services.FeatureSecrets}

	// Fully licensed (valid or in grace period)
	if h.licenseService.IsFeatureEnabled(services.FeatureSecrets) {
	...
	if h.licenseService.IsReadOnly() && h.licenseService.HasFeature(services.FeatureSecrets) {
		if isWrite {
			detail := map[string]string{"feature": services.FeatureSecrets, "reason": "license_expired"}
	...
	if !h.licenseService.IsLicensed() {
		...
		response.WriteError(w, http.StatusPaymentRequired, response.ErrCodeLicenseRequired,
			"Secrets feature is not included in your license", featureDetail)
	}
	return false
}
...
func (h *SecretsHandler) CreateSecret(w http.ResponseWriter, r *http.Request) {
	...
	// Check license validity (write operation)
	if h.licenseService != nil && !h.checkSecretsLicense(w, true) {
		return
	}
...
func (h *SecretsHandler) ListSecrets(w http.ResponseWriter, r *http.Request) {
	...
	// Check license validity (read operation)
	if h.licenseService != nil && !h.checkSecretsLicense(w, false) {
		return
	}
