...
// TestCommitFiles_Integration tests committing files to GitHub
// This test is skipped by default and only runs when GITHUB_INTEGRATION_TEST=true
// It requires real GitHub credentials to be configured
func TestCommitFiles_Integration(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping integration test in short mode")
	}

	// Skip if not explicitly enabled
	// To run: GITHUB_INTEGRATION_TEST=true go test -v
	// This prevents accidental commits during normal testing
	t.Skip("Integration test requires real GitHub credentials - skip by default")

	ctx := context.Background()

	// Setup (would require real credentials)
	k8sClient := fake.NewSimpleClientset()
	client := NewGitHubClient(k8sClient)

	// This test would:
	// 1. Create a GitHub client with real credentials
	// 2. Commit a test file to a test repository
	// 3. Verify the commit SHA is returned
	// 4. Verify the file exists in the repository
	// 5. Clean up the test commit

	_ = ctx
	_ = client

	// Actual implementation would go here if running in integration environment
}

// TestCommitFile_SingleFile tests the convenience method for single file commits
func TestCommitFile_SingleFile(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping integration test in short mode")
	}

	// Skip by default - requires real GitHub setup
	t.Skip("Integration test requires real GitHub credentials - skip by default")
	...
	// This test would verify that CommitFile correctly commits a single file
	// and is a convenience wrapper for CommitFiles with one file

	_ = ctx
	_ = client
}

// TestGetDefaultBranch_Integration tests getting the default branch
func TestGetDefaultBranch_Integration(t *testing.T) {
	...
	// This test would:
	// 1. Get default branch for a known repository
	// 2. Verify it returns "main" or "master"
