...
export function Sidebar({
  ...
  const coreNavItems: NavItem[] = [
    ...
    // Secrets: enterprise-only, only shown when user has secrets:get permission
    // Use === true to avoid flashing the nav item while permission is loading (undefined)
    ...(isEnterprise() && canViewSecrets === true ? [{ id: "secrets" as NavTab, label: "Secrets", icon: KeyRound, to: "/secrets" }] : []),
  ];
