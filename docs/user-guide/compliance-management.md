...
### Why can't I see the Compliance menu?

The Compliance feature requires:
- Enterprise license enabled
- Your role includes `compliance` resource permissions

Contact your administrator to enable access.

### Why does enforcement action update fail?

Possible causes:
- **403 Forbidden**: Your role lacks `compliance/update` permission
- **500 Internal Server Error**: Kubernetes RBAC missing `update` verb for constraints

**Solution:** Ask your administrator to:
1. Add `compliance, *, *, allow` to your project role
2. Verify the `knodex-gatekeeper-reader` ClusterRole includes `update` and `patch` verbs

### What happens when I change from dryrun to deny?

- Existing resources remain unchanged
- New resource creations that violate the policy are blocked
- Updates to existing resources that introduce violations are blocked

### Can I change enforcement for multiple constraints at once?

Currently, enforcement actions must be changed one constraint at a time. Bulk updates are planned for a future release.

### How often are violations updated?

Violations are refreshed every 30 seconds via WebSocket updates. Manual refresh is available by clicking the refresh icon.

---

**Next:** [Deployment Modes](../deployment-modes/) | **Previous:** [Project Management](../project-management/)
